...
from predictor.prepare_features import process_stock  # updated module
from predictor.train_model import train_one         # updated module
...
class PredictView(APIView):
    def get(self, request):
        ...
        # If missing, generate features and model
        ...
        if not os.path.exists(feature_file) or not os.path.exists(model_file):
            ...
            process_stock(stock_obj)
            train_one(feature_file)

        # Validate existence after generation
        if not os.path.exists(feature_file):
            return JsonResponse({"error": f"Features for {ticker} not found after generation."}, status=404)
        if not os.path.exists(model_file):
            return JsonResponse({"error": f"Model for {ticker} not found after training."}, status=404)
        ...
        # Generate chart of last 20 movements
